...
const SiapPanel = dynamic(() => import('@/components/SiapPanel'), { ssr: false });
...
export default function Dashboard() {
  ...
  useEffect(() => {
    (window as unknown as { gaiaWatchFlight?: (f: WatchedFlight) => void }).gaiaWatchFlight = (f) => {
      if (!f?.icao24) return;
  ...
  const [siapPlayback, setSiapPlayback] = useState<{ inspectorId: string; index: number; playing: boolean } | null>(null);
  const [siapPlaybackDot, setSiapPlaybackDot] = useState<{ lng: number; lat: number; inspectorId: string; heading?: number; vehicle?: string } | null>(null);
  const [siapTrail, setSiapTrail] = useState<{ type: 'LineString'; coordinates: [number, number][]; color?: string } | null>(null);
  const [siapSpeed, setSiapSpeed] = useState<1 | 2>(1);
  ...
  const handleMouseCoords = useCallback((coords: { lat: number; lng: number }) => {
    ...
    geocodeTimer.current = setTimeout(async () => {
      ...
      } catch (e) { console.warn('[GAIA] Suppressed error:', e instanceof Error ? e.message : e); }
  ...
  const handleRightClick = useCallback(async (coords: { lat: number; lng: number }) => {
    ...
    } catch (e) { console.warn('[GAIA] Suppressed error:', e instanceof Error ? e.message : e); } finally { setDossierLoading(false); }
  }, []);
  const bearing = useCallback((a: [number, number], b: [number, number]) => {
    const toRad = (d: number) => d * Math.PI / 180; const toDeg = (r: number) => r * 180 / Math.PI;
    const dLng = toRad(b[0]-a[0]); const lat1 = toRad(a[1]); const lat2 = toRad(b[1]);
  ...
  const handleSiapRoutePlay = useCallback((ins: SiapInspector, mode: 'moto'|'a pie') => {
    ...
    if (first) {
      ...
      setSiapPlaybackDot({ lng: first.lng, lat: first.lat, inspectorId: ins.id, heading, vehicle: ins.vehicle });
  ...
  const handleEntityClick = useCallback((entity: any) => {
    if (entity?.type === 'siap') {
      const ins = siapInspectors.find(i=>i.id===entity.inspectorId);
      if (ins) { const pt = ins.points.find(pp=>pp.id===entity.id); if (pt) handleSiapPointClick(pt, ins); }
      return;
    }
    if (entity?.type === 'siap-route') {
      const ins = siapInspectors.find(i=>i.id===entity.inspectorId);
      if (ins) { setSiapSelected(ins.id); handleSiapRoutePlay(ins, 'moto'); }
      return;
    }
  ...
  }, [siapInspectors, handleSiapPointClick, handleSiapRoutePlay]);

  // ── GAIA SIAP — animación suave direccional con velocidad 1x/2x ──
  useEffect(() => {
    if (!siapPlayback?.playing) return;
    const ins = siapInspectors.find(i => i.id === siapPlayback.inspectorId);
    if (!ins) return;
    const startPt = ins.points[siapPlayback.index];
    const endPt = ins.points[siapPlayback.index + 1];
    if (!startPt || !endPt) return;
    const base = ins.vehicle === 'moto' ? 1800 : 2800;
    const duration = base / siapSpeed;
    let raf = 0;
    ...
    const animate = (now: number) => {
      ...
      const lng = startPt.lng + (endPt.lng - startPt.lng) * e;
      const lat = startPt.lat + (endPt.lat - startPt.lat) * e;
      const heading = bearing([startPt.lng, startPt.lat], [endPt.lng, endPt.lat]);
      setSiapPlaybackDot({ lng, lat, inspectorId: ins.id, heading, vehicle: ins.vehicle });
      const trailCoords = ins.points.slice(0, siapPlayback.index + 1).map(p => [p.lng, p.lat] as [number, number]);
      trailCoords.push([lng, lat] as [number, number]);
      setSiapTrail({ type: 'LineString', coordinates: trailCoords, color: ins.color });
      if (t < 1) raf = requestAnimationFrame(animate);
    };
    raf = requestAnimationFrame(animate);
    return () => cancelAnimationFrame(raf);
  }, [siapPlayback, siapInspectors, siapSpeed, bearing]);
  ...
  const handleExportGeoJSON = useCallback(() => {
    downloadFile(
      `gaia-aoi-${new Date().toISOString().slice(0, 10)}.geojson`,
  ...
  const fetchEndpoint = useCallback(async (
    ...
    try {
      ...
      console.warn('[GAIA] Suppressed error:', e instanceof Error ? e.message : e);
  ...
  useEffect(() => {
    ...
    return loadCameraCatalog(cameras => {
      ...
      setBackendStatus('connected');
    }, () => console.warn('[GAIA] Camera catalogue load failed; bounded retry scheduled'));
  }, [activeLayers.cctv]);
  ...
  // ── GAIA SDK — Intelligence Fusion Layer ──
    ...
    <main className="fixed inset-0 w-full h-full bg-[var(--bg-void)] overflow-hidden">
      ...
      <AnimatePresence>
        {showSplash && (
          ...
          >
            ...
            {/* ── GAIA · altura.com.ec badge — top-left ── */}
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 0.6 }}
            ...
            {/* ── GAIA title — letter-by-letter stagger ── */}
            <div className="flex items-center gap-[2px] mb-3 z-[2]">
              {'GAIA'.split('').map((letter, i) => (
                <motion.span
                  key={i}
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, delay: 2.5 }} className={`absolute top-4 z-[200] pointer-events-none flex flex-col`} style={{ left: isMobile ? '24px' : '64px', right: '24px' }}>
        <div className="flex items-center gap-3 w-fit">
          <img src="/arco-color.png" alt="GAIA Arco" className="w-10 h-10 md:w-11 md:h-11 shrink-0 object-contain drop-shadow-[0_0_8px_rgba(23,167,210,0.5)] rounded-md bg-white/5 p-1" />
          <div className="flex flex-col items-start gap-0.5">
            <h1 className="text-lg md:text-xl font-bold tracking-[0.4em] text-[#17A7D2] font-mono">GAIA</h1>
            <span className="text-[9px] md:text-[10px] font-mono tracking-[0.2em] opacity-90 uppercase text-[#17A7D2]">SISTEMA DE INSPECCIÓN TERRITORIAL SIAP</span>
          </div>
        </div>
      ...
      {/* ── GAIA SIAP Panel (derecha) ── */}
      {showSiap && siapInspectors.length > 0 && !isMobile && (
        <div className="absolute top-[64px] right-3 z-40 hidden lg:block">
          <SiapPanel
            inspectors={siapInspectors}
            selectedId={siapSelected}
            onSelect={setSiapSelected}
            onPointClick={handleSiapPointClick}
            onPlayRoute={handleSiapRoutePlay}
            onStop={() => { setSiapPlayback(null); setSiapPlaybackDot(null); setSiapTrail(null); setActiveRoute(null); }}
            playback={siapPlayback}
            setPlayback={setSiapPlayback}
            speed={siapSpeed}
                ...
                <div className="px-3 pb-3">
                  <div className="flex items-center justify-between mb-2">
                    <span className="hud-text text-[10px] text-[var(--text-primary)]">
                      {mobilePanel === 'layers' ? 'LAYERS & STATS' : mobilePanel === 'markets' ? 'MARKETS & INTEL' : mobilePanel === 'intel' ? 'INTEL FEED' : mobilePanel === 'recon' ? 'GAIA RECON' : mobilePanel === 'remote' ? 'WORLD REMOTE' : 'SEARCH'}
                    </span>
