...
interface OsirisMapProps {
  ...
  /** SIAP inspection playback — dot moving along route */
  siapPlayback?: { lng: number; lat: number; inspectorId: string; heading?: number; vehicle?: string } | null;
...
function OsirisMap({ data, activeLayers, onEntityClick, onMouseCoords, onRightClick, onViewStateChange, flyToLocation, projection = 'globe', terrainEnabled = false, terrainRetry = 0, terrainFocus = 0, onTerrainStatusChange, onRetryMap, mapStyle = 'dark', sweepData, scanTargets = [], demoMode = false, theme = 'core', drawnPolygons = [], arcgisLayers = [], drawMode = null, onDrawComplete, onDrawProgress, onDrawCancel, drawCommand = null, onMapCenter, route = null, userLocation = null, followUser = false, onFollowInterrupt, navigating = false, aircraftAirports = {}, siapPlayback = null, siapTrail = null, siapFilter = null }: OsirisMapProps) {
  ...
  useEffect(() => {
    ...
    map.on('load', () => {
      ...
      // ── SIAP vehicle icons (moto / a pie) — 24px canvas, white silhouette on transparent ──
      const createSiapIcon = (id: string, draw: (ctx: CanvasRenderingContext2D, s: number) => void) => {
        if (map.hasImage(id)) return;
        const s = 28;
      ...
      createSiapIcon('siap-moto', (ctx, s) => {
        // moto: two wheels + body + rider
        ctx.fillStyle = '#FFFFFF'; ctx.strokeStyle = '#FFFFFF'; ctx.lineWidth = 1.8; ctx.lineCap = 'round'; ctx.lineJoin = 'round';
        // wheels
        ctx.beginPath(); ctx.arc(s*0.30, s*0.72, 4.5, 0, Math.PI*2); ctx.stroke();
        ctx.beginPath(); ctx.arc(s*0.72, s*0.72, 4.5, 0, Math.PI*2); ctx.stroke();
        // frame
        ctx.beginPath(); ctx.moveTo(s*0.30, s*0.72); ctx.lineTo(s*0.45, s*0.50); ctx.lineTo(s*0.68, s*0.50); ctx.lineTo(s*0.72, s*0.72); ctx.stroke();
        ctx.beginPath(); ctx.moveTo(s*0.45, s*0.50); ctx.lineTo(s*0.38, s*0.38); ctx.stroke();
        // handle
        ctx.beginPath(); ctx.moveTo(s*0.68, s*0.50); ctx.lineTo(s*0.74, s*0.44); ctx.stroke();
        // rider head + body
        ctx.beginPath(); ctx.arc(s*0.50, s*0.30, 3.2, 0, Math.PI*2); ctx.fill();
        ctx.beginPath(); ctx.moveTo(s*0.50, s*0.34); ctx.lineTo(s*0.47, s*0.52); ctx.stroke();
        ctx.beginPath(); ctx.moveTo(s*0.47, s*0.42); ctx.lineTo(s*0.58, s*0.46); ctx.stroke();
      });
      createSiapIcon('siap-walk', (ctx, s) => {
        // persona caminando
        ctx.fillStyle = '#FFFFFF'; ctx.strokeStyle = '#FFFFFF'; ctx.lineWidth = 1.8; ctx.lineCap = 'round'; ctx.lineJoin = 'round';
        // head
        ctx.beginPath(); ctx.arc(s*0.50, s*0.28, 3.5, 0, Math.PI*2); ctx.fill();
        // body
        ctx.beginPath(); ctx.moveTo(s*0.50, s*0.32); ctx.lineTo(s*0.50, s*0.58); ctx.stroke();
        // arms swinging
        ctx.beginPath(); ctx.moveTo(s*0.50, s*0.40); ctx.lineTo(s*0.38, s*0.50); ctx.stroke();
        ctx.beginPath(); ctx.moveTo(s*0.50, s*0.40); ctx.lineTo(s*0.62, s*0.48); ctx.stroke();
        // legs stepping
        ctx.beginPath(); ctx.moveTo(s*0.50, s*0.58); ctx.lineTo(s*0.38, s*0.74); ctx.stroke();
        ctx.beginPath(); ctx.moveTo(s*0.50, s*0.58); ctx.lineTo(s*0.62, s*0.72); ctx.stroke();
      });
      createSiapIcon('siap-moto-play', (ctx, s) => {
        ctx.fillStyle = '#FFFFFF'; ctx.strokeStyle = '#FFFFFF'; ctx.lineWidth = 2; ctx.lineCap = 'round';
        ctx.beginPath(); ctx.arc(s*0.30, s*0.72, 5, 0, Math.PI*2); ctx.stroke();
        ctx.beginPath(); ctx.arc(s*0.72, s*0.72, 5, 0, Math.PI*2); ctx.stroke();
        ctx.beginPath(); ctx.moveTo(s*0.30, s*0.72); ctx.lineTo(s*0.45, s*0.45); ctx.lineTo(s*0.70, s*0.45); ctx.lineTo(s*0.72, s*0.72); ctx.stroke();
        ctx.beginPath(); ctx.arc(s*0.50, s*0.26, 4, 0, Math.PI*2); ctx.fill();
      });
      createSiapIcon('siap-walk-play', (ctx, s) => {
        ctx.fillStyle = '#FFFFFF'; ctx.strokeStyle = '#FFFFFF'; ctx.lineWidth = 2; ctx.lineCap = 'round';
        ctx.beginPath(); ctx.arc(s*0.50, s*0.26, 4, 0, Math.PI*2); ctx.fill();
        ctx.beginPath(); ctx.moveTo(s*0.50, s*0.30); ctx.lineTo(s*0.50, s*0.60); ctx.stroke();
        ctx.beginPath(); ctx.moveTo(s*0.50, s*0.40); ctx.lineTo(s*0.35, s*0.52); ctx.stroke();
        ctx.beginPath(); ctx.moveTo(s*0.50, s*0.40); ctx.lineTo(s*0.65, s*0.50); ctx.stroke();
      ...
      // ── SIAP — Recorridos inspectores (Manta) ──
      map.addLayer({ id: 'siap-route-casing', type: 'line', source: 'siap-routes', layout: { 'line-cap': 'round', 'line-join': 'round' }, paint: { 'line-color': '#001014', 'line-width': ['interpolate',['linear'],['zoom'], 5, 5, 12, 9], 'line-opacity': 0.85 }});
      map.addLayer({ id: 'siap-route-line', type: 'line', source: 'siap-routes', layout: { 'line-cap': 'round', 'line-join': 'round' }, paint: { 'line-color': ['get','color'], 'line-width': ['interpolate',['linear'],['zoom'], 5, 3, 12, 6], 'line-opacity': 0.95 }});
      map.addLayer({ id: 'siap-route-dash', type: 'line', source: 'siap-routes', layout: { 'line-cap': 'round', 'line-join': 'round' }, paint: { 'line-color': '#FFFFFF', 'line-width': ['interpolate',['linear'],['zoom'], 5, 1.2, 12, 2], 'line-opacity': 0.35, 'line-dasharray': [2, 4] }});
      ...
      map.addLayer({ id: 'siap-point-vehicle', type: 'symbol', source: 'siap-points', layout: { 'icon-image': ['match',['get','vehicle'],'moto','siap-moto','a pie','siap-walk','siap-moto'], 'icon-size': ['interpolate',['linear'],['zoom'], 10, 0.55, 14, 0.85, 16, 1.05], 'icon-allow-overlap': true, 'icon-ignore-placement': true, 'icon-anchor': 'center' }, paint: { 'icon-opacity': 0.95 }});
      map.addLayer({ id: 'siap-point-num', type: 'symbol', source: 'siap-points', layout: { 'text-field': ['to-string',['get','orden']], 'text-size': 8, 'text-font': ['JetBrains Mono Bold','Open Sans Bold'], 'text-allow-overlap': true, 'text-offset': [0, -1.2] }, paint: { 'text-color': '#FFFFFF', 'text-halo-color': '#000', 'text-halo-width': 1 }});
      map.addLayer({ id: 'siap-point-label', type: 'symbol', source: 'siap-points', minzoom: 12, layout: { 'text-field': ['get','titulo'], 'text-size': 9, 'text-font': ['Open Sans Regular'], 'text-offset': [0, 1.6], 'text-max-width': 14, 'text-allow-overlap': false }, paint: { 'text-color': '#E8E6E0', 'text-halo-color': '#000', 'text-halo-width': 1.2, 'text-opacity': 0.85 }});
      map.addLayer({ id: 'siap-playback-trail-casing', type: 'line', source: 'siap-playback-trail', layout: { 'line-cap': 'round', 'line-join': 'round' }, paint: { 'line-color': '#001014', 'line-width': ['interpolate',['linear'],['zoom'], 5, 6, 12, 11], 'line-opacity': 0.85 }});
      map.addLayer({ id: 'siap-playback-trail', type: 'line', source: 'siap-playback-trail', layout: { 'line-cap': 'round', 'line-join': 'round' }, paint: { 'line-color': ['get','color'], 'line-width': ['interpolate',['linear'],['zoom'], 5, 4, 12, 7], 'line-opacity': 0.9 }});
      map.addLayer({ id: 'siap-playback-trail-dash', type: 'line', source: 'siap-playback-trail', layout: { 'line-cap': 'round', 'line-join': 'round' }, paint: { 'line-color': '#FFFFFF', 'line-width': 2, 'line-opacity': 0.5, 'line-dasharray': [1, 3] }});
      // flecha direccional sobre la ruta recorrida
      map.addLayer({ id: 'siap-playback-arrows', type: 'symbol', source: 'siap-playback-trail', layout: { 'symbol-placement': 'line', 'symbol-spacing': 60, 'icon-image': 'siap-arrow', 'icon-size': 0.7, 'icon-allow-overlap': true, 'icon-ignore-placement': true }, paint: { 'icon-opacity': 0.9 }});
      ...
      // etiqueta origen → destino en ruta SIAP
      map.addLayer({ id: 'siap-route-dir-label', type: 'symbol', source: 'siap-routes', minzoom: 11, layout: { 'symbol-placement': 'line-center', 'text-field': ['concat',['get','inspectorName'],' → ',['to-string',['get','pointsCount']],' pts'], 'text-size': 10, 'text-font': ['JetBrains Mono Bold','Open Sans Bold'], 'text-allow-overlap': false }, paint: { 'text-color': ['get','color'], 'text-halo-color': '#000', 'text-halo-width': 1.5 }});
    ...
    ['siap-point-dots','siap-route-line'].forEach(l => {
    ...
    // ── SIAP point click ──
    map.on('click', 'siap-point-dots', e => {
      const p = (e.features?.[0]?.properties as any);
      if (!p) return;
      try { p.fotos = JSON.parse(p.fotos || '[]'); } catch { p.fotos = []; }
      onEntityClick?.({ type: 'siap', ...p, lng: e.lngLat.lng, lat: e.lngLat.lat });
      const fotosHtml = (p.fotos || []).slice(0,2).map((u: string) => `<img src="${u}" style="width:100%;border-radius:6px;margin-top:6px;max-height:140px;object-fit:cover;" />`).join('');
      const videoHtml = p.video ? `<video src="${p.video}" controls style="width:100%;border-radius:6px;margin-top:6px;max-height:160px;"></video>` : '';
      const impactCol = p.impacto==='critico'?'#D32F2F':p.impacto==='alto'?'#FF6F00':p.impacto==='medio'?'#FFC400':'#00C853';
      new maplibregl.Popup({ closeButton: true, maxWidth: '360px', offset: 14 }).setLngLat(e.lngLat).setHTML(
        `<div style="background:rgba(8,16,32,0.96);backdrop-filter:blur(16px);border:1px solid rgba(23,167,210,0.25);border-radius:10px;padding:14px;font-family:'JetBrains Mono',monospace;">
          <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;">
            <span style="background:${p.color};color:#fff;font-size:10px;font-weight:700;padding:2px 6px;border-radius:4px;">${p.inspectorId}</span>
            <span style="color:#5CCAF0;font-size:10px;">${p.inspectorName} · ${p.vehicle}</span>
            <span style="margin-left:auto;padding:2px 6px;border-radius:4px;font-size:9px;font-weight:700;color:#fff;background:${impactCol};">${(p.impacto||'').toUpperCase()}</span>
          </div>
          <div style="color:#E8E6E0;font-size:13px;font-weight:700;line-height:1.3;">${p.titulo}</div>
          <div style="color:#9BA7C2;font-size:11px;margin-top:6px;line-height:1.4;">${p.descripcion}</div>
          <div style="display:flex;gap:6px;margin-top:8px;font-size:9px;color:#5CCAF0;">
            <span>#${p.orden} · ${p.estado}</span><span>·</span><span>${new Date(p.fecha).toLocaleDateString('es-EC')}</span>
          </div>
          ${fotosHtml}
          ${videoHtml}
        </div>`
    ...
    map.on('click', 'siap-route-line', e => {
      const p = (e.features?.[0]?.properties as any);
      if (!p) return;
      onEntityClick?.({ type: 'siap-route', inspectorId: p.inspectorId, inspectorName: p.inspectorName });
    });
  ...
  // ── SIAP Inspecciones (Manta) ──
  useEffect(() => {
    if (!mapReady) return;
    const inspectors: any[] = (data as any).siap_inspectors || [];
    const filtered = siapFilter ? inspectors.filter((i: any) => i.id === siapFilter) : inspectors;
    const routeFeatures = filtered.map((ins: any) => ({
      type: 'Feature' as const,
      geometry: ins.route,
      properties: { color: ins.color, inspectorId: ins.id, inspectorName: ins.name, vehicle: ins.vehicle, pointsCount: ins.points?.length || 0 },
    }));
    const pointFeatures = filtered.flatMap((ins: any) => (ins.points || []).map((pt: any) => ({
      type: 'Feature' as const,
      geometry: { type: 'Point' as const, coordinates: [pt.lng, pt.lat] as [number, number] },
      properties: {
        id: pt.id, orden: pt.orden, titulo: pt.titulo, descripcion: pt.descripcion,
        lng: pt.lng, lat: pt.lat, impacto: pt.impacto, estado: pt.estado, fecha: pt.fecha,
        fotos: JSON.stringify(pt.fotos || []), video: pt.video || '', thumb: pt.thumb || '',
        color: ins.color, inspectorId: ins.id, inspectorName: ins.name, inspectorAvatar: ins.avatar, vehicle: ins.vehicle,
      },
    })));
    setGeo('siap-routes', routeFeatures);
    setGeo('siap-points', pointFeatures);
  }, [mapReady, (data as any).siap_inspectors, siapFilter, setGeo]);

  useEffect(() => {
    if (!mapReady) return;
    if (!siapPlayback) { setGeo('siap-playback', []); return; }
    const ins = ((data as any).siap_inspectors || []).find((i: any) => i.id === siapPlayback.inspectorId);
    const col = ins?.color || '#17A7D2';
    const veh = (siapPlayback.vehicle as string) || (ins?.vehicle as string) || 'moto';
    const heading = typeof siapPlayback.heading === 'number' ? siapPlayback.heading : 0;
    setGeo('siap-playback', [{ type: 'Feature' as const, geometry: { type: 'Point' as const, coordinates: [siapPlayback.lng, siapPlayback.lat] as [number, number] }, properties: { color: col, inspectorId: siapPlayback.inspectorId, vehicle: veh, heading } }]);
  }, [mapReady, siapPlayback, (data as any).siap_inspectors, setGeo]);

  useEffect(() => {
    if (!mapReady) return;
    if (!siapTrail || !siapTrail.coordinates?.length) { setGeo('siap-playback-trail', []); return; }
    setGeo('siap-playback-trail', [{ type: 'Feature' as const, geometry: siapTrail, properties: { color: siapTrail.color || '#17A7D2' } }]);
  }, [mapReady, siapTrail, setGeo]);
